...
function desc = imagedesc( I, imdesc, show )
    ...
    if(~isfield(imdesc,'iscuboid') || ~isfield(imdesc,'fun_desc') || ...
        ...
        error('imdesc lacking crucial field.');
    end;
    ...
    %%% apply descriptor, should give nxp or nxpxr vector, where n=size(I,3);
    if imdesc.hasChn == false
        if( (nd==2 && ~iscuboid) || (nd==3 && iscuboid))
            desc = imagedesc1( I, imdesc );
        else
            desc = fevalArrays( I, @imagedesc1, imdesc );
            dnd=ndims(desc); desc=permute(desc,[dnd 2:(dnd-1) 1]);
        end
    else
        if( (nd==3 && ~iscuboid) || (nd==4 && iscuboid))
            desc = imagedesc1( I, imdesc );
        else
            desc = fevalArrays( I, @imagedesc1, imdesc );
            dnd=ndims(desc); desc=permute(desc,[dnd 2:(dnd-1) 1]);
...
function desc = imagedesc1( I, imdesc )
    if imdesc.normalizeIm == true
        % similar to blow normalization, most vals fall between [-1,1]
        I = imNormalize(I,1)/4; 
    else
    ...
    if( isempty(par_jitter) ) % no jitter
        temp = imdesc.par_desc;
        desc = imdesc.fun_desc( I, temp{:} ); % apply basic descriptor
