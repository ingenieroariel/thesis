...
% descJT  -  Nxpxr, where N is number of samples, p is dimensionality, 
%            and r is jitter
%
% See also JITTER_DIST, KMEANS2

function [IDX,IDXr,C,sumd] = jitter_kmeans( descJT, k, prm )

    %%% get input args   (NOT SUPPORTED:  distance, emptyaction, start )
    dfs = {'nTrial', 1, 'maxIter', 200,  'display', 0, 'randstate', [],...
        'outFrac', 0, 'minCl', 1};
    [replicates, maxiter, display, randstate, outlierfrac, minCsize] = ...
        getPrmDflt(prm,dfs);
    ...
    for i=1:replicates
        tic
        if (display)
            msg = ['kmeans iteration ' num2str(i) ' of ' ...
                   num2str(replicates) ', step: '];
            disp(msg);
        end;
        [IDX,IDXr,C,sumd,niters] = ...
            kmeans2_main(descJT,k,noutliers,minCsize,maxiter,display);
        if (sum(sumd)<sum(bestsumd))
            bestIDX = IDX; bestIDXr=IDXr; bestC = C; bestsumd = sumd;
        end
        if (display && replicates>1)
            msg = ['\nCompleted kmeans iteration ' num2str(i) ' of ' ...
                   num2str(replicates) ';  number of kmeans steps= ' ...
                   num2str(niters) ';  sumd=' num2str(sum(sumd)) '\n'];
            fprintf(msg); toc,
        end;
    end
    
    IDX = bestIDX; IDXr = bestIDXr; C = bestC; sumd = bestsumd; k = max(IDX);
    msg = ['Final number of clusters = ' num2str( k ) ...
           ';  sumd=' num2str(sum(sumd))]; 
    if (display), disp(msg); end;    
    
    % sort IDX to have biggest clusters have lower indicies (no need to alter
    % IDXr)
    clustercounts = zeros(1,k);
    for i=1:k, clustercounts(i) = sum( IDX==i ); end
    [~, order] = sort( -clustercounts );  C = C(order,:);  sumd = sumd(order);
    IDX2 = IDX;
    for i=1:k, IDX2(IDX==order(i))=i; end; IDX = IDX2; 
...
function [IDX, IDXr, mind] = jitter_kmeans_dist( descJT, C )
    ...
    DS = fevalArrays( descJT, @pdist2, C , 'euclidean');
    [D IDXr] = min( DS, [], 3 );  
    [mind IDX] = min( D, [], 2 );
