...
function [ V, Imasked ] = cuboid_display( I, cuboids_rs, subs, show )
    ...
    [ cuboids, cuboid_starts, cuboid_ends, subs] = cuboid_extract( I, cuboids_rs, subs, 2 );
    n = size( subs, 1 );  
...
%     V = V / max(V(:));
    ...
    if (show) 
        if (nd==2)                 
            ...
            figure(show);    clf;  montage2( I,struct('showLines',1) );            
            figure(show+1);  clf;  montage2( V,struct('showLines',1) );
            figure(show+2);  clf;  montages2( cell2array(cuboids) ); %, {0,0,[0,1]} );    
