...
function [R,subs,vals,cuboids,V] = stfeatures( I, sigma, tau, periodic, thresh, maxn, ...
    ...
    if isempty(R)
        subs = []; vals = []; R = zeros(size(I)); return;
    end
    ...
    [subs, vals] = nonMaxSuprWin( subs,vals,1+cuboids_rs,size(I)-cuboids_rs,[],maxn);
    if ~isempty(subs)
    ...
    if( isempty(subs) ), cuboids=[]; return; end; %#ok<ISMT>
    cuboids = cell2array( cuboid_extract( I, cuboids_rs, subs, 0 ) );
    if( nargout<5 && show==0 ), return; end;
    V = cuboid_display( I_sm, cuboids_rs_sm, subs_sm);
    
    %%% optionally display
    if( show ) 
        disp( ['Create visualization; ' num2str(size(vals,1)) ' features' ]);
        figure(show); clf; montage2( I_sm,struct('showLines',1) );
        figure(show+1); clf; montage2( R,struct('showLines',1,'extraInfo',1) ); colormap jet;
        figure(show+2); clf; montage2( V,struct('showLines',1,'extraInfo',1)  );
        figure(show+3); clf; montages2( cuboids, struct('showLines',1) );
    end;
