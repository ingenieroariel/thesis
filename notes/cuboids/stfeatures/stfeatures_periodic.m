...
function Lquad = stfeatures_periodic( I, sigma, tau )
    ...
    L = gaussSmooth( I, [sigma,sigma,0], 'valid' ); %*sigma;
    if numel(L) == 0
        Lquad = [];
        return;
    end
    ...
    [feven,fodd] = filterGabor1d(2*tau,2*tau,.5/tau);
    feven = permute( feven, [3 1 2] ); fodd = permute( fodd, [3 1 2] );  
    Leven = convnFast( L, feven, 'valid' );
    Lodd = convnFast( L, fodd, 'valid' );
    Lquad = Leven.^2 + Lodd.^2;
    clear L Leven Lodd feven fodd
    Lquad = arrayToDims( Lquad, size(I) );
